...
import { Users, UserPlus, Edit2, Trash2, X, Save, AlertCircle } from 'lucide-react';
...
const TechnicianManagement = () => {
  ...
  const validationSchema = Yup.object({
    ...
    specialization: Yup.string()
      ...
      .oneOf(specializations, 'Invalid specialization'),
    ...
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      try {
        if (editingTechnician) {
          const { data } = await axios.put(`/technicians/${editingTechnician._id}`, values);
          if (data.success) {
            toast.success('Technician updated successfully');
          }
        } else {
          const { data } = await axios.post('/technicians', values);
          if (data.success) {
